...
beforeEach(() => {
	...
	// resolved aspect ratios are cached on a static map keyed by url; clear it so tests that reuse
	// a url don't leak a previously-resolved ratio into each other
	;(EmbedShapeUtil as any).resolvedAspectRatios?.clear()
})
