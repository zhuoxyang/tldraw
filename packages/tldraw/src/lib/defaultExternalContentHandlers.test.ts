...
describe('defaultHandleExternalUrlContent', () => {
	...
	it('ignores a data: url rather than making a giant text shape', async () => {
		// data: urls can carry multi-MB base64 payloads. Falling back to a text
		// shape there would create an unusable blob, so we skip them entirely.
		editor = new TestEditor()
		const { opts, addToast } = makeOpts()
		registerDefaultExternalContentHandlers(editor, opts)
		...
		await expect(
			defaultHandleExternalUrlContent(
				editor,
				{ url: 'data:image/svg+xml;base64,PHN2Zz48L3N2Zz4=', point: { x: 0, y: 0 } },
				opts
			)
		).resolves.not.toThrow()
